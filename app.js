...
function showCallStatusMessage() {
    ...
    // 添加到消息列表并渲染
    messages.push(callStatusMessage);
    renderMessage(callStatusMessage);
    scrollToBottom();
...
function showCallEndMessage() {
    ...
    // 添加到消息列表并渲染
    messages.push(callEndMessage);
    renderMessage(callEndMessage);
    scrollToBottom();
    ...
    // 通过WebSocket发送给其他用户
    if (isRealtimeEnabled && window.realtimeClient) {
        window.realtimeClient.sendMessage(callEndMessage);
    }
}
...
function updateCallActions() {
    ...
    if (!joinCallBtn || !leaveCallBtn || !callStatusIndicator) return;
    
    if (isInCall) {
        // 当前用户在通话中
        joinCallBtn.style.display = 'none';
        leaveCallBtn.style.display = 'block';
        callStatusIndicator.innerHTML = '<i class="fas fa-phone-alt"></i><span>正在通话中...</span>';
        callStatusIndicator.style.color = '#10b981';
    } else {
...
function handleCallInvite(data) {
    ...
    if (modal && callerNameElement) {
        // 设置呼叫者姓名
        callerNameElement.textContent = data.callerName || '未知用户';
        ...
        console.log('📞 来电提示模态框已显示');
        
        // 添加调试信息
        console.log('📞 模态框状态:', {
            display: modal.style.display,
            zIndex: modal.style.zIndex,
            callerName: callerNameElement.textContent
        });
        
        // 播放来电铃声（可选）
        // playIncomingCallSound();
    } else {
        console.error('❌ 来电提示模态框元素未找到');
        // 如果模态框不存在，使用Toast提示
        showToast(`${data.callerName} 邀请您加入语音通话`, 'info', 5000);
    }
}
...
function addCurrentUserToParticipants() {
    // 首先检查是否已存在相同用户名的用户
    const existingUserByName = participants.find(p => p.name === currentUsername);
    const existingUserById = participants.find(p => p.userId === currentUserId);
    
    if (existingUserByName && !existingUserById) {
        // 如果存在相同用户名但不同ID，更新现有用户的ID和状态
        existingUserByName.userId = currentUserId;
        existingUserByName.status = 'online';
        existingUserByName.lastSeen = Date.now();
        console.log('🔄 合并相同用户名用户:', currentUsername);
    } else if (!existingUserByName && !existingUserById && currentUsername) {
        // 如果不存在相同用户名，添加新用户
        participants.push({
            userId: currentUserId,
        ...
        // 如果存在相同ID的用户，更新状态
        existingUserById.status = 'online';
        existingUserById.lastSeen = Date.now();
        console.log('🔄 更新现有用户状态:', currentUsername);
    }
...
function setUsername() {
    ...
    if (window.realtimeClient && !window.realtimeClient.localMode) {
        ...
        if (existingUser) {
            // 使用现有的用户ID，但更新为新的ID以确保一致性
            currentUserId = existingUser.userId || existingUser.id;
            currentUsername = username;
            ...
            // 确保使用正确的ID字段
            if (existingUser.id && !existingUser.userId) {
                existingUser.userId = existingUser.id;
            }
            
            // 更新消息显示中的"(我)"标识
            updateMessagesOwnership();
            console.log('🔄 使用现有用户:', username, 'ID:', currentUserId);
        } else {
            // 添加新用户到参与者列表
            participants.push({
                userId: currentUserId,
            ...
            console.log('➕ 创建新用户:', username, 'ID:', currentUserId);
        }
...
function renderMessage(message) {
    ...
    messageDiv.className = `message ${message.type}-message${message.isAIQuestion ? ' ai-question-message' : ''}${message.isCallStatus ? ' call-status-message' : ''}${message.isCallEnd ? ' call-end-message' : ''}`;
    ...
    if (message.isCallStatus) {
        avatarColor = '#3b82f6';
        avatarContent = '<i class="fas fa-phone" style="color: white;"></i>';
    ...
    if (message.isCallEnd) {
        avatarColor = '#6b7280';
        avatarContent = '<i class="fas fa-phone-slash" style="color: white;"></i>';
        messageDiv.style.cssText = `
            text-align: center;
            margin: 10px 0;
            padding: 8px 16px;
        ...
        messageDiv.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                <i class="fas fa-phone-slash" style="color: #6b7280;"></i>
                <span>${message.text}</span>
            </div>
        `;
...
async function testAudioOutput() {
    try {
        ...
        console.log('✅ 音频输出测试完成');
        showToast('音频输出测试完成，如果听到声音说明音频系统正常', 'success');
        
    } catch (error) {
        console.error('❌ 音频输出测试失败:', error);
        showToast('音频输出测试失败: ' + error.message, 'error');
    }
}
...
async function testMicrophoneInput() {
    try {
        ...
        console.log('✅ 麦克风输入测试完成，请说话测试');
        showToast('麦克风测试已启动，请说话测试（3秒后自动停止）', 'info');
...
window.testMicrophoneInput = testMicrophoneInput;